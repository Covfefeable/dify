...
class AppService:
    ...
    def copy_app_to_tenant(
        ...
        """Copy an app to another workspace the current account can edit."""
        if not account:
        ...
        with Session(db.engine, expire_on_commit=False) as session:
            ...
            try:
                ...
                if result.status == ImportStatus.FAILED:
                    session.rollback()
                    raise ValueError(result.error or "Failed to import app.")
                if result.status == ImportStatus.PENDING:
                    session.rollback()
                    raise ValueError("App import is pending dependency resolution.")

                session.commit()
                app = session.get(App, result.app_id) if result.app_id else None
                if not app:
                    raise ValueError("Copied app not found.")
                return app
            finally:
