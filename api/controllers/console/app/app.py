...
class AppCopyToTenantPayload(CopyAppPayload):
...
class AppCopyToTenantApi(Resource):
    ...
    @console_ns.doc(description="Create a copy of an existing application in another workspace")
    @console_ns.doc(params={"app_id": "Application ID to copy"})
    @console_ns.expect(console_ns.models[AppCopyToTenantPayload.__name__])
    @console_ns.response(201, "App copied successfully", console_ns.models[AppDetailWithSite.__name__])
    @console_ns.response(403, "Insufficient permissions")
    @setup_required
    @login_required
    ...
    def post(self, current_user: Account, app_model: App):
        args = AppCopyToTenantPayload.model_validate(console_ns.payload or {})
        target_membership = db.session.scalar(
            select(TenantAccountJoin)
            .where(
                TenantAccountJoin.tenant_id == args.target_tenant_id,
                TenantAccountJoin.account_id == current_user.id,
            )
            .limit(1)
        )
        if not target_membership:
            raise Forbidden("You don't have access to the target workspace.")
        if not TenantAccountRole.is_editing_role(target_membership.role):
            raise Forbidden("You need editor role or above in the target workspace.")

        app = AppService().copy_app_to_tenant(
            app_model=app_model,
