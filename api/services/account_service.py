...
class TenantService:
    ...
    def dissolve_tenant(tenant: Tenant, operator: Account) -> None:
        """Dissolve tenant"""
        TenantService.check_member_permission(tenant, operator, None, "remove", session=db.session)
        tenant_to_delete = db.session.get(Tenant, tenant.id)
    ...
    def invite_new_member(
        ...
        *,
        ...
        if not account:
            ...
            create_workspace_required = dify_config.INIT_WORKSPACE_WHEN_INVITE
