...
import type { CopyToTenantModalProps } from '@/app/components/app/copy-to-tenant'
...
import { copyApp, copyAppToTenant, exportAppConfig, updateAppInfo } from '@/service/apps'
...
const CopyToTenantModal = dynamic(() => import('@/app/components/app/copy-to-tenant'), {
  ssr: false,
})
...
type AppCardOperationsMenuProps = {
  ...
  shouldShowCopyToTenantOption: boolean
...
function AppCardOperationsMenu({
  ...
  const hasCreateExportGroup = shouldShowDuplicateOption || shouldShowCopyToTenantOption || shouldShowExportOption
  ...
  return (
    <>
      ...
      {shouldShowCopyToTenantOption && (
        <DropdownMenuItem className="gap-2 px-3" onClick={e => handleMenuAction(e, onCopyToTenant)}>
...
export function AppCardActionBar({ app, onRefresh }: AppCardActionBarProps) {
  ...
  const handleShowCopyToTenantModal = useCallback(() => {
    setIsOperationsMenuOpen(false)
    queueMicrotask(() => {
  ...
  const onCopyToTenant: CopyToTenantModalProps['onConfirm'] = async ({ name, targetTenantId }) => {
    try {
      ...
      toast.success(t('copyToTenant.done', { ns: 'app' }))
      setNeedRefresh('1')
      onRefresh?.()
      onPlanInfoChanged()
    }
    catch {
      toast.error(t('copyToTenant.failed', { ns: 'app' }))
    }
  }
  ...
  const shouldShowCopyToTenantOption = canCreateApp
  ...
  const shouldShowOperationsMenu = shouldShowEditOption || shouldShowDuplicateOption || shouldShowCopyToTenantOption || shouldShowExportOption || shouldShowSwitchOption || shouldShowAccessControlOption || shouldShowAccessConfigOption || shouldShowDeleteOption
                ...
                {systemFeatures.webapp_auth.enabled
                  ? (
                      <AppCardOperationsMenuContent
                        ...
                        shouldShowCopyToTenantOption={shouldShowCopyToTenantOption}
                        ...
                        onCopyToTenant={handleShowCopyToTenantModal}
...
export function AppCard({ app, onlineUsers = [], onRefresh, onOpenTagManagement = () => {} }: AppCardProps) {
  ...
  function handleShowCopyToTenantModal() {
    setIsOperationsMenuOpen(false)
    queueMicrotask(() => {
      setShowCopyToTenantModal(true)
    })
