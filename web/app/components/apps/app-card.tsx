...
import { copyAppToTenant } from '@/service/apps'
...
function AppCardOperationsMenu({
  ...
  const hasCreateExportGroup =
    shouldShowDuplicateOption || shouldShowCopyToTenantOption || shouldShowExportOption
    ...
    <>
      ...
      {shouldShowCopyToTenantOption && (
        <DropdownMenuItem
          ...
          onClick={(e) => handleMenuAction(e, onCopyToTenant)}
        >
          <span className="system-sm-regular text-text-secondary">
...
export const AppCardActionBar = memo(
  ({
    ...
    const handleShowCopyToTenantModal = useCallback(() => {
      setIsOperationsMenuOpen(false)
      queueMicrotask(() => setShowCopyToTenantModal(true))
    }, [setIsOperationsMenuOpen])
    ...
    const onCopyToTenant: CopyToTenantModalProps['onConfirm'] = async ({ name, targetTenantId }) => {
      try {
        await copyAppToTenant({ appID: app.id, name, target_tenant_id: targetTenantId })
